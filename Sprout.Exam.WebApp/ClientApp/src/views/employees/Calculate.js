...
<div className='form-group col-md-6'>
  ...
  <input type='number' className='form-control' id='inputAbsentDays4' step="1" onChange={this.handleChange.bind(this)} value={this.state.absentDays} name="absentDays" placeholder='Absent Days' />
</div> :
<div className='form-group col-md-6'>
  <label htmlFor='inputWorkDays4'>Worked Days: </label>
  <input type='number' className='form-control' id='inputWorkDays4' step="1" onChange={this.handleChange.bind(this)} value={this.state.workedDays} name="workedDays" placeholder='Worked Days' />
</div>
}
</div>
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async calculateSalary() {
    ...
    this.setState({ loadingCalculate: false,netIncome: Number(data.payload).toFixed(2) });
  }
  ...
  async getEmployee(id) {
    ...
    if(data.statusCode === 200){
        this.setState({ id: data.payload.id,fullName: data.payload.fullName,birthdate: data.payload.birthdate,tin: data.payload.tin,typeId: data.payload.typeId, loading: false,loadingCalculate: false });
    }
    else{
